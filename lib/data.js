import { similarListings } from "./similar.js";
...
export async function relatedProperties(p) {
  if (!configured) return similarListings(demoProperties, p);
  const db = await supabase();
  let q = db
    .from("market_properties")
    .select("*")
  ...
  if (p.listing_type === "rent")
    q = q.eq("rent_period", p.rent_period || "month");
  const { data, error } = await q
    .order("created_at", { ascending: false })
    .limit(100);
  if (error) throw Error("Unable to load similar listings");
  return similarListings(data, p);
}
